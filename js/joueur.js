...
// Cellule
const SOLIDE = new Set(["B", "Be"]); // Brique, Béton, Echelle (pour ne pas tomber)
const ECHELLE = "E";
const CORDE = "C";
...
export class Joueur {
  constructor(
    ...
    this.vitesse = 2; // Vitesse de déplacement
    this.gravite = 0.8;
  ...
  deplacementHorizontal(direction) {
    ...
    const nCol =
      direction === -1
        ? Math.floor((nX - this.w + 32) / TAILLE_CELLULE)
        : Math.floor((nX + this.w) / TAILLE_CELLULE);
    const nRow = this.row;
    const nCell = cellule(this.niveau, nCol, nRow);

    if (!estSolide(nCell) || estEchelle(nCell)) {
      this.x = nX;
    }
  }

  monterEchelle() {
    // On peut monter si sur une échelle ou si la cellule au dessus est un échelle
    const col = this.col;
    const row = this.row;
    ...
    if (
      (estEchelle(cellCourante) || estEchelle(cellDessus)) &&
      row < this.niveau.length - 1
    ) {
      this.y -= this.vitesse;
  ...
  descendreEchelle() {
    ...
    if (estEchelle(cellDessous) && !estSolide(cellDessous)) {
      this.y += this.vitesse;
      this.vy = 0; // Annule la gravité
    }
  }
